  ...
  async function captureSnapshot(): Promise<void> {
    snapshotBusy = true;
    snapshotStatus = 'Capturing snapshot…';
    snapshotsError = null;
    try {
      ...
      snapshotsError = buildErrorMessage({ error: err, fallback: 'Unable to capture snapshot.' });
      snapshotStatus = null;
    } finally {
      snapshotBusy = false;
    }
  ...
  async function deleteSnapshot(id: string): Promise<void> {
    ...
    try {
      ...
      snapshotsError = buildErrorMessage({ error: err, fallback: 'Unable to delete snapshot.' });
    }
  }
  ...
  async function downloadSnapshot(id: string): Promise<void> {
    ...
    try {
      ...
      snapshotsError = buildErrorMessage({ error: err, fallback: 'Unable to download snapshot.' });
      snapshotStatus = null;
    }
  }
</script>
...
<div class="space-y-4 text-sm text-surface-200 w-full">
  <div class="grid gap-3 sm:grid-cols-3 lg:grid-cols-4 auto-rows-fr">
    <div class="border border-surface-700/60 bg-surface-950/35 p-3">
      <p class="text-micro uppercase tracking-[0.3em] text-surface-500">Snapshots</p>
      <p class="text-xl font-semibold text-surface-50">{snapshots.length}</p>
      <p class="text-xs text-surface-400">Total captured on device</p>
    </div>
    {#if snapshots.length}
      {@const latest = snapshots[0]}
      <div class="border border-surface-700/60 bg-surface-950/35 p-3">
        <p class="text-micro uppercase tracking-[0.3em] text-surface-500">Last capture</p>
    ...
    {:else}
      <div class="border border-surface-700/60 bg-surface-950/35 p-3">
        ...
        <p class="text-xs text-surface-400">Capture to populate</p>
      </div>
    {/if}
    <div class="border border-surface-700/60 bg-surface-950/35 p-3">
      <p class="text-micro uppercase tracking-[0.3em] text-surface-500">Retention</p>
  ...
  <div class="grid gap-3 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] auto-rows-fr">
    <div class="space-y-3 border border-surface-700/60 bg-surface-950/35 p-4">
      <header class="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p class="text-xs uppercase tracking-[0.3em] text-surface-500">Retention policy</p>
          <p class="text-sm text-surface-400">Applies to future snapshots captured on this device.</p>
        </div>
    ...
    <div class="space-y-3 border border-surface-700/60 bg-surface-950/35 p-4">
      <p class="text-xs uppercase tracking-[0.3em] text-surface-500">Capture snapshot</p>
      <label class="space-y-1 text-sm">
  ...
  <div class="border border-surface-700/60 bg-surface-950/35 p-4">
    <p class="text-xs uppercase tracking-[0.3em] text-surface-500">Archive list</p>
    {#if snapshotsLoading && snapshots.length === 0}
      <p class="mt-2 text-sm text-surface-500">Loading snapshots…</p>
    {:else if snapshots.length === 0}
      <p class="mt-2 text-sm text-surface-500">No snapshots reported.</p>
    {:else}
