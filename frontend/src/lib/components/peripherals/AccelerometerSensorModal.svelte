<script lang="ts">
  ...
  onMount(() => {
    ...
    sampleTimer = setInterval(() => {
      ...
      lastHistoryAtMs = now;
      historyX = [...historyX, accel.x].slice(-HISTORY_MAX);
      historyY = [...historyY, accel.y].slice(-HISTORY_MAX);
      historyZ = [...historyZ, accel.z].slice(-HISTORY_MAX);
    }, 40);
  });
    ...
    <div class="space-y-4">
      <div class="rounded-xl border border-surface-800 bg-surface-950/70 p-4 shadow-inner shadow-black/30">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div class="space-y-1">
            <p class="text-micro uppercase tracking-[0.3em] text-surface-500">3-axis accelerometer</p>
      ...
      <div class="rounded-xl border border-surface-800 bg-surface-900/70 p-4 shadow-inner shadow-black/20">
        <p class="text-[0.7rem] uppercase tracking-[0.3em] text-surface-500">Acceleration (g)</p>
        <dl class="mt-3 space-y-2 text-sm text-surface-100">
          <div class="flex items-center justify-between rounded border border-surface-800/70 bg-surface-950/50 px-3 py-2">
            ...
            <dd class="font-semibold" style="color: var(--axis-x)">{formatAxis(accel.x)}</dd>
          </div>
          <div class="flex items-center justify-between rounded border border-surface-800/70 bg-surface-950/50 px-3 py-2">
            <dt class="text-micro uppercase tracking-[0.25em] text-surface-500">Y</dt>
            <dd class="font-semibold" style="color: var(--axis-y)">{formatAxis(accel.y)}</dd>
          </div>
          <div class="flex items-center justify-between rounded border border-surface-800/70 bg-surface-950/50 px-3 py-2">
            <dt class="text-micro uppercase tracking-[0.25em] text-surface-500">Z</dt>
            <dd class="font-semibold" style="color: var(--axis-z)">{formatAxis(accel.z)}</dd>
          </div>
        </dl>
      </div>
      ...
      <div class="rounded-xl border border-surface-800 bg-surface-900/70 p-4 shadow-inner shadow-black/20">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <p class="text-[0.7rem] uppercase tracking-[0.3em] text-surface-500">Motion</p>
          <p class={`text-xs font-semibold ${isMoving ? 'text-success-300' : 'text-surface-500'}`}>
            {isMoving ? 'Moving' : 'Still'} · {motionG.toFixed(3)}g
          </p>
        </div>
      </div>
    ...
    <div class="space-y-3">
      ...
      {#if historyX.length > 1}
        <div class="grid gap-3">
          <MiniSparkline label="X" series={historyX} unit="g" color="var(--axis-x)" precision={3} heightClass="h-28" />
          <MiniSparkline label="Y" series={historyY} unit="g" color="var(--axis-y)" precision={3} heightClass="h-28" />
          <MiniSparkline label="Z" series={historyZ} unit="g" color="var(--axis-z)" precision={3} heightClass="h-28" />
        </div>
      {:else}
