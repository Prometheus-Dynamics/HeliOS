<script lang="ts">
  ...
  const mag = $derived<ImuAxes>(imu?.mag ?? { x: 0, y: 0, z: 0 });
  const hasMagSample = $derived(
    Boolean(imu?.mag) &&
      (Boolean(imu?.hasSample) ||
        Boolean(imu?.updatedAt) ||
        [mag.x, mag.y, mag.z].some((value) => Number.isFinite(value) && Math.abs(value) > 1e-6))
  );
  const magnitude = $derived(computeMagnitude(mag));
  ...
  const sourceLabel = $derived(imu?.sources?.magnetometer ?? null);
  ...
  $effect(() => {
    if (!hasMagSample || !imu?.mag) return;
    const now = Date.now();
    if (now - lastHistoryAtMs < 40) return;
    lastHistoryAtMs = now;
    historyX = [...historyX, mag.x].slice(-HISTORY_MAX);
    historyY = [...historyY, mag.y].slice(-HISTORY_MAX);
    historyZ = [...historyZ, mag.z].slice(-HISTORY_MAX);
  });
  ...
  function computeStatus(): string {
    ...
    if (!imu.mag) return 'No magnetometer data';
    return imu.hasSample ? 'Active' : 'Idle · awaiting samples';
  }

  function computeMagnitude(axes: ImuAxes): number {
    ...
    <div class="space-y-4">
      <div class="rounded-xl border border-surface-800 bg-surface-950/70 p-4 shadow-inner shadow-black/30">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div class="space-y-1">
            <p class="text-micro uppercase tracking-[0.3em] text-surface-500">3-axis magnetometer</p>
      ...
      <div class="rounded-xl border border-surface-800 bg-surface-900/70 p-4 shadow-inner shadow-black/20">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <p class="text-[0.7rem] uppercase tracking-[0.3em] text-surface-500">3-axis magnetometer (uT)</p>
          <div class="flex flex-wrap items-center gap-2 text-xs text-surface-300">
            <span class="rounded-full border border-surface-700 bg-surface-950/60 px-2 py-1">Field {formatAxis(magnitude)} uT</span>
          </div>
        </div>
        <dl class="mt-3 space-y-2 text-sm text-surface-100">
          <div class="flex items-center justify-between rounded border border-surface-800/70 bg-surface-950/50 px-3 py-2">
            ...
            <dd class="font-semibold" style="color: var(--axis-x)">{formatAxis(mag.x)}</dd>
          </div>
          <div class="flex items-center justify-between rounded border border-surface-800/70 bg-surface-950/50 px-3 py-2">
            <dt class="text-micro uppercase tracking-[0.25em] text-surface-500">Y</dt>
            <dd class="font-semibold" style="color: var(--axis-y)">{formatAxis(mag.y)}</dd>
          </div>
          <div class="flex items-center justify-between rounded border border-surface-800/70 bg-surface-950/50 px-3 py-2">
            <dt class="text-micro uppercase tracking-[0.25em] text-surface-500">Z</dt>
            <dd class="font-semibold" style="color: var(--axis-z)">{formatAxis(mag.z)}</dd>
          </div>
        </dl>
      </div>
    ...
    <div class="space-y-3">
      ...
      {#if hasMagSample}
        <div class="grid gap-3">
          <MiniSparkline label="X" series={historyX} unit="uT" color="var(--axis-x)" precision={2} heightClass="h-28" />
          <MiniSparkline label="Y" series={historyY} unit="uT" color="var(--axis-y)" precision={2} heightClass="h-28" />
          <MiniSparkline label="Z" series={historyZ} unit="uT" color="var(--axis-z)" precision={2} heightClass="h-28" />
        </div>
      {:else}
        <div class="rounded border border-surface-800/70 bg-surface-900/50 p-3 text-sm text-surface-400">
          No magnetometer samples yet.
        </div>
      {/if}
    </div>
  {/snippet}
