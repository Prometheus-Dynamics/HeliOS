<script lang="ts">
  ...
  import type { PipelineUiAccordion, PipelineUiItem, PipelineUiNodeDescriptor } from '$lib/features/pipelines/pipelineUiTypes';
...
<div data-ui-item role="group" class={wrapperClass} ondragover={(event) => onDragOver(event, item)} ondragleave={(event) => onDragLeave(event, item)} ondrop={(event) => onDrop(event, item)}>
  ...
  {/if}
  <div class="rounded border border-surface-800/70 bg-surface-900/40">
    <details open={editMode ? true : item.defaultOpen ?? false}>
      <summary class="cursor-pointer px-3 py-2 text-xs font-semibold text-surface-100">
        {item.title}
        {#if item.description}
          <span class="ml-2 text-micro-tight text-surface-500">{item.description}</span>
        {/if}
      </summary>
      <div class="px-3 pb-3">
        <Renderer
        ...
        {#if editMode && item.id}
          <div
            ...
            role="group"
            aria-label="Drop inside accordion"
