...
export function intervalToFps(interval: Interval | undefined | null): number | null {
  ...
  const numerator = Number((interval as any).numerator ?? 0);
  const denominator = Number((interval as any).denominator ?? 0);
...
export function mediaFormatMatches(mode: Mode | null | undefined, target: any): boolean {
...
export function frameRateToFps(rate: any): number | null {
...
export function fpsToFrameRate(fps: number | null): any | null {
