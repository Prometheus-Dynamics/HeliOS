  ...
  $effect(() => {
    runTunePipelineReset({
      currentPipelineId: get(selectedPipeline)?.id ?? null,
  ...
  $effect(() => {
    ...
    if (get(activeTab) !== 'tune' || tunePerformanceTab !== 'metrics') {
    ...
    const scheduleReconnect = (wanted: TuneMetricsStreamRef[], connect: () => void) => {
      ...
      reconnectTimer = window.setTimeout(() => {
        ...
        tuneMetricsSnapshots = wanted.map((ref) => buildTuneMetricsSnapshot(ref, null, { pipelineId: get(selectedPipeline)?.id ?? null }));
  ...
  function scheduleTuneGlobalAutoSave(): void {
    ...
    if (get(pipelineUpdatesReady)) return;
  ...
  const { scheduleTuneStreamAutoApply, applyTuneStreamOverridesFor } = untrack(() =>
    createTuneStreamOverrideRuntime({
      browser,
      getStreamUpdatesReadyById: (streamId) => Boolean(get(streamUpdatesReadyById)[streamId]),
      getSelectedPipeline: () => get(selectedPipeline) ?? null,
