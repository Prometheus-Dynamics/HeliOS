...
OS_DESC=${OS_DESC:-1}
GADGET_UDC_WAIT_SEC=${GADGET_UDC_WAIT_SEC:-120}
...
wait_for_udc() {
  ...
  i=0
  max_tries=$((GADGET_UDC_WAIT_SEC * 2))
  [ "$max_tries" -gt 0 ] 2>/dev/null || max_tries=240
  log "waiting for UDC up to ${GADGET_UDC_WAIT_SEC}s"
...
# Add a serial control plane in both USB configs so either host-selected
# configuration exposes a CDC ACM endpoint.
if [ "$USE_ACM" = "1" ]; then
  mkdir -p "$G/functions/acm.usb0"
  ln -s "$G/functions/acm.usb0" "$CFG1/acm.usb0"
  mkdir -p "$G/functions/acm.usb1"
  ln -s "$G/functions/acm.usb1" "$CFG2/acm.usb1"
  log "added acm.usb0 -> c.1 and acm.usb1 -> c.2"
fi
