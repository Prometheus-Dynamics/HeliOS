...
extern "C" fn camera_request_completed_cb(ptr: *mut core::ffi::c_void, req: *mut libcamera_request_t) {
    let mut state = unsafe { &*(ptr as *const Mutex<ActiveCameraState<'_>>) }
        .lock()
        .unwrap();
    // The callback can race with shutdown/dequeue paths where the request has
    // already been removed from the in-flight map. In that case we ignore the
    // stale completion instead of panicking and aborting the process.
    let req = match state.requests.remove(&req) {
        Some(req) => req,
        None => return,
    };
