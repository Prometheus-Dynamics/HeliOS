...
async fn apply_dns_config(config: &DnsConfig) -> Result<()> {
...
async fn start_dhcp_client(interface: &str) -> Result<()> {
...
async fn stop_dhcp_client(interface: &str) -> Result<()> {
...
mod tests {
    ...
    fn dns_config_roundtrip() {
        ...
        set_test_dns_path(Some(path.clone()));
        ...
        runtime.block_on(async {
            ...
            apply_dns_config(&config).await.expect("write dns");
            let read = read_dns_config().await.expect("read dns");
