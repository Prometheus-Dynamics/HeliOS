...
fn cv_aruco_decode_quads_hamming_finalize_detections(
    ...
    if let Some(dict_name) = dictionary.as_aruco_name() {
        ...
        let mut detections: Vec<ArucoDetection2D> = detections
            .into_iter()
            .filter_map(|mut det| {
                if max_hamming_u32 != u32::MAX
                    && let Some(best) = det.best_distance
                    && best > max_hamming_u32
                {
                    return None;
                }
                if det.bits.is_none() {
                ...
                if canonicalize {
                    det = det.canonicalize();
                }
                Some(det)
            })
            .collect();
        detections = filter_detections_id_range(&detections, min_id, max_id);
        return Ok(filter_detections_in_frame(&detections, frame_w, frame_h));
    }
    ...
    out = filter_detections_id_range(&out, min_id, max_id);
    Ok(filter_detections_in_frame(&out, frame_w, frame_h))
}
...
fn cv_aruco_decode_quads_hamming(
    ...
    let frame = expect_cpu_frame(frame, "decode_quads_hamming", Some(exec_ctx))?;
    ...
    let quads: Vec<Quad> = if min_quad_side_px > 0.0 { quads.into_iter().filter(|q| quad_min_side_px(q) >= min_quad_side_px).collect() } else { quads };
    ...
    let decoded = if quads.is_empty() {
        ...
        DECODE_QUAD_SCRATCH.with(|quad_scratch| {
            ...
            decode_quads_aruco_with_config_no_bits(&frame, cv_quads, sample_scale_u32, &dict, &decode_cfg)
        })
    } else {
        let Some(family) = dictionary.as_apriltag_family() else {
            return Err(NodeError::InvalidInput("unknown tag dictionary".into()));
        ...
        DECODE_FAMILY_SCRATCH.with(|family_scratch| {
            ...
            DECODE_QUAD_SCRATCH.with(|quad_scratch| {
                ...
                decode_quads_with_config_no_bits(&frame, cv_quads, sample_scale_u32, &family_scratch.family, &decode_cfg)
            })
        })
    };
    ...
    let finalized = if refined.is_empty() {
        ...
        let max_hamming_u32 = if max_hamming >= 0 { u32::try_from(max_hamming).unwrap_or(0) } else { u32::MAX };
        let mut detections: Vec<ArucoDetection2D> = refined
            .into_iter()
            .filter_map(|mut det| {
                if max_hamming_u32 != u32::MAX
                    && let Some(best) = det.best_distance
            ...
            .collect();
        detections = filter_detections_id_range(&detections, min_id, max_id);
        filter_detections_in_frame(&detections, frame_w, frame_h)
    } else {
        let Some(family) = dictionary.as_apriltag_family() else {
            return Err(NodeError::InvalidInput("unknown tag dictionary".into()));
        };
        let family_impl = family.into_family();
        let mut out: Vec<ArucoDetection2D> = Vec::with_capacity(refined.len());
        for mut det in refined {
            if det.bits.is_none() {
                det.bits = family_impl.bit_grid(det.id as usize);
            }
            det = det.canonicalize();
        ...
        out = filter_detections_id_range(&out, min_id, max_id);
        filter_detections_in_frame(&out, frame_w, frame_h)
    };
