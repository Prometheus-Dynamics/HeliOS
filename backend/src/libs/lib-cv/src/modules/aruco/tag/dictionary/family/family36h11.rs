...
pub struct Family36H11;

impl Family36H11 {
    pub const NAME: &'static str = "36h11";
}

impl ArucoTagDecoding for Family36H11 {
...
impl Family36H11 {
    // "h11" is the *minimum* Hamming distance between codes; the maximum number of correctable
    // bit errors is floor((d-1)/2).
    const HAMMING: u8 = 5;
    const BORDER_SIZE: u8 = 1;
    const TOTAL_WIDTH: u8 = 8;
    const DATA_WIDTH: u8 = 6;
    const DATA_BITS_LOCATION: [(u8, u8); 36] = [
        (0, 0),
        (1, 0),
        (2, 0),
        (3, 0),
