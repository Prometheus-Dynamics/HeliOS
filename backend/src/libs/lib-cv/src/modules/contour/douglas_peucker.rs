...
#[inline(always)]
fn with_rdp_scratch_i32<R>(f: impl FnOnce(&mut RdpScratchI32) -> R) -> R {
...
///
/// This mirrors `approx_poly_dp_closed_fast_into` but keeps the contour in `i32` space until
/// the final simplified vertices are chosen, which avoids converting every dense contour point
/// to `f32` in the ArUco fast candidate path.
...
fn approx_poly_dp_open_with_buffers_i32_into(points: &[Point<i32>], epsilon: f32, stack: &mut Vec<(usize, usize)>, marker_epoch: &mut Vec<u32>, marker_gen: &mut u32, out: &mut Vec<Point<i32>>) {
    let len = points.len();
    if len < 3 || epsilon <= 0.0 {
        out.clear();
        out.extend_from_slice(points);
...
fn approx_poly_dp_closed_fast_i32_with_scratch(points: &[Point<i32>], epsilon: f32, out: &mut Vec<Point<i32>>, scratch: &mut RdpScratchI32) {
    let n = points.len();
    if n < 3 || epsilon <= 0.0 {
        out.clear();
        out.extend_from_slice(points);
...
fn approximate_farthest_pair_axis_extrema_i32(points: &[Point<i32>]) -> (usize, usize) {
    let n = points.len();
    if n == 0 {
        return (0, 0);
    }
