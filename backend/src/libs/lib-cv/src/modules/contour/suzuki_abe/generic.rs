...
pub(super) fn suzuki_abe_with_scratch<T>(image: &GrayImage, image_values: &mut Vec<i32>) -> Vec<Contour<T>>
where
    T: Num + NumCast + Copy + PartialEq,
{
    ...
    // Track per-pixel border ids; 0 means background or unvisited foreground.
    ...
    with_row_offsets(width, height, |row_offsets| {
            ...
            if inner_row && width > 2 {
                ...
                if x == 0 {
                    ...
                    if let Some((adj, border_type)) = if state == 0 && x > 0 && unsafe { *image_data.get_unchecked(idx - 1) } == 0 {
                        ...
                        let converted = convert_points(&contour_points);
                        contours.push(Contour::new(converted, border_type, parent));
                    }

                    let state = unsafe { *image_values.get_unchecked(idx) };
                    if state != 0 {
                        parent_border_num = state.unsigned_abs() as usize;
                    }
                    x = 1;
                }
...
fn convert_points<T: Num + NumCast>(points: &[Point<i32>]) -> Vec<Point<T>> {
    points.iter().map(|p| Point::new(T::from(p.x).unwrap(), T::from(p.y).unwrap())).collect()
}
