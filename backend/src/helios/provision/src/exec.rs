...
pub fn resize_partition(plan: &PartPlan, disk: &str, sector_bytes: u64, logger: &mut Logger) -> Result<()> {
    ...
    if current_end_mib >= plan.end_mib.saturating_sub(1) {
        ...
        if let Err(err) = reread_partition_table(disk, logger) {
            logger.log(format!("warn: partition table reread failed after resizepart on {}: {}", disk, err));
        }
...
pub fn create_partition(plan: &PartPlan, disk: &str, logger: &mut Logger) -> Result<()> {
    ...
    if geometry_mismatch {
        ...
        if let Err(err) = reread_partition_table(disk, logger) {
            logger.log(format!("warn: partition table reread failed after rm on {}: {}", disk, err));
        }
    ...
    if !already_exists {
        ...
        if let Err(err) = reread_partition_table(disk, logger) {
            logger.log(format!("warn: partition table reread failed after mkpart on {}: {}", disk, err));
        }
    }
