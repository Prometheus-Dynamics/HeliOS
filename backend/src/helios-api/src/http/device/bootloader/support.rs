...
use super::types::{BOOT_MOUNT, BootloaderConfig, BootloaderStatus, CONFIG_PATH, DEFAULT_BOOT_PARTITION, DEFAULT_UPDATE_FILE, DEFAULT_UPDATE_SIG};

pub(super) fn bootloader_status() -> (BootloaderStatus, Option<String>) {
    let config = load_config();
    bootloader_status_with_config(&config)
}

pub(super) fn load_config() -> BootloaderConfig {
    ...
    if !config.boot_partition.exists() {
        if let Some(resolved) = resolve_boot_partition() {
...
pub(super) fn bootloader_status_with_config(config: &BootloaderConfig) -> (BootloaderStatus, Option<String>) {
...
pub(super) fn stage_update(config: &BootloaderConfig) -> Result<(), Box<ApiError>> {
...
pub(super) fn resolve_boot_partition() -> Option<PathBuf> {
    ...
    if let Some(data_dev) = canonicalize_existing_path("/dev/disk/by-label/DATA") {
        if let Some(disk) = disk_from_partition_device(&data_dev) {
...
pub(super) fn canonicalize_existing_path(path: &str) -> Option<String> {
...
pub(super) fn disk_from_partition_device(dev: &str) -> Option<String> {
    match dev {
...
pub(super) fn partition_device(disk: &str, part: u32) -> PathBuf {
...
fn is_mountpoint(path: &Path) -> bool {
    ...
    for line in mounts.lines() {
        ...
        if let Some(mount_point) = mount_point {
            if mount_point == target.to_string_lossy() {
                return true;
            }
        }
    }
