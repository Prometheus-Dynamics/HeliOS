...
pub(crate) async fn list_peer_streams_for_peer(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
...
pub(crate) async fn proxy_peer_stream_format(State(state): State<AppState>, Path((id, stream_id)): Path<(String, String)>, raw_query: RawQuery, headers: HeaderMap) -> impl IntoResponse {
...
pub(crate) async fn proxy_peer_stream_preview(State(state): State<AppState>, Path((id, stream_id)): Path<(String, String)>, raw_query: RawQuery, headers: HeaderMap) -> impl IntoResponse {
...
pub(crate) async fn proxy_peer_stream_frame(State(state): State<AppState>, Path((id, stream_id)): Path<(String, String)>, raw_query: RawQuery, headers: HeaderMap) -> impl IntoResponse {
...
pub(crate) async fn sync_peer_pipelines(State(state): State<AppState>, Path(id): Path<String>, Json(req): Json<PeerPipelineSyncRequest>) -> impl IntoResponse {
    let peers = snapshot_peers(&state).await;
    let Some(peer) = peers.into_iter().find(|peer| peer.id == id) else {
        return (StatusCode::NOT_FOUND, Json(PeerError { error: "peer not found".to_string() })).into_response();
    };
...
pub(super) async fn collect_peer_stream_inventory(state: &AppState, peers: &[PeerInfo]) -> (Vec<PeerRemoteStreamSummary>, Vec<PeerResourceError>) {
...
async fn collect_peer_streams_for_peer(state: &AppState, peer: &PeerInfo) -> Result<Vec<PeerRemoteStreamSummary>, String> {
    ...
    for stream in remote_streams {
        ...
        summaries.push(PeerRemoteStreamSummary {
            ...
            outputs,
            imu_output_keys,
            preview_format: stream.preview_format,
...
mod tests {
    ...
    fn parse_peer_scoped_ref_requires_both_segments() {
        assert_eq!(parse_peer_scoped_ref("peer:robot:camera"), Some(("robot".to_string(), "camera".to_string())));
    ...
    fn build_peer_v1_url_appends_v1_once() {
        assert_eq!(build_peer_v1_url("http://robot.local", "/streams").unwrap(), "http://robot.local/v1/streams");
        assert_eq!(build_peer_v1_url("http://robot.local/v1", "/streams").unwrap(), "http://robot.local/v1/streams");
    }

    #[test]
    fn build_peer_v1_url_rejects_empty_base() {
        assert_eq!(build_peer_v1_url("", "/streams").unwrap_err(), "peer api_base_url missing");
    }
}
