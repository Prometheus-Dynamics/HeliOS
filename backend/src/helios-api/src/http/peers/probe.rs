...
pub(super) async fn infer_discovered_integration_kind(
    peers: &PeersServiceState,
    host: &str,
    seen_ports: &std::collections::BTreeSet<u16>,
    timeout_ms: u64,
) -> Option<PeerIntegrationKind> {
    if seen_ports.contains(&5800) {
    ...
    if seen_ports.contains(&5801) || seen_ports.contains(&5800) {
        let limelight_management =
            probe_http_any(peers, &format!("http://{host}:5801"), timeout_ms).await;
        if limelight_management.ok {
            return Some(PeerIntegrationKind::LimelightOs);
        }
        let limelight_stream =
            probe_mjpegish(peers, &format!("http://{host}:5800/stream.mjpeg"), timeout_ms).await;
...
pub(super) fn apply_integration_defaults(
    integration: &mut PeerIntegrationMetadata,
    host: &str,
) {
...
fn integration_defaults(kind: &PeerIntegrationKind, host: &str) -> IntegrationDefaults {
    match kind {
        ...
        PeerIntegrationKind::Helios | PeerIntegrationKind::Custom => IntegrationDefaults {
            management_url: None,
            stream_url: None,
            stream_urls: Vec::new(),
        },
...
pub(super) fn normalize_api_base(
    raw: Option<&str>,
    ...
    if trimmed.is_empty() {
        let host = device_ip
            ...
            .ok_or_else(|| "api_base_url or device_ip is required".to_string())?;
...
pub(super) fn derive_endpoints(api_base_url: &str) -> Vec<PeerEndpoint> {
    let candidate = if api_base_url.contains("://") {
...
pub(crate) async fn photonvision_discover_streams(
    State(state): State<AppState>,
    Json(req): Json<PhotonvisionDiscoverStreamsRequest>,
) -> impl IntoResponse {
    ...
    let streams = discover_photonvision_streams(
...
pub(crate) async fn probe_peer(
    State(state): State<AppState>,
    Json(req): Json<PeerProbeRequest>,
) -> impl IntoResponse {
    ...
    if let Some(management_url) = req
        .management_url
        .as_deref()
        .filter(|value| !value.trim().is_empty())
    {
    ...
    if matches!(req.kind, PeerIntegrationKind::Photonvision) {
        ...
        if let Some(host) = host {
            let discovered =
                discover_photonvision_streams(peers, &host, 1181, 8, timeout_ms.min(2500)).await;
            let discovered_urls =
                discovered.iter().map(|stream| stream.url.clone()).collect::<Vec<_>>();
            if !discovered_urls.is_empty() {
                candidate_stream_urls = merge_urls(candidate_stream_urls, discovered_urls);
            }
            photonvision = Some(PhotonvisionDiscoverStreamsResponse {
                host: host.clone(),
                streams: discovered,
            });
        }
    ...
    (StatusCode::OK, Json(PeerProbeResponse {
        kind: req.kind,
        api,
        management,
        stream,
...
async fn probe_nt4(host: &str, port: u16, timeout_ms: u64) -> Nt4PeerProbe {
    ...
    if !settings.subscriptions_enabled {
        return Nt4PeerProbe {
            ...
            ok: false,
            roots: Vec::new(),
            error: Some("nt4 subscriptions are disabled in device settings".into()),
        };
    }

    let entry = match crate::nt4::pool()
        .get_or_connect(host, port, "HeliOS-peers-probe")
        .await
    {
        Ok(entry) => entry,
        Err(err) => {
    ...
    if let Err(err) = entry
        .wait_ready(std::time::Duration::from_millis(timeout_ms))
        .await
    {
    ...
    let topics = entry
        .list_topics_prefix("/", std::time::Duration::from_millis(250))
        .await
        .unwrap_or_default();
    let mut roots = topics
        .iter()
        .filter_map(|topic| topic.trim_start_matches('/').split('/').next())
        .filter(|seg| !seg.is_empty() && *seg != ".schema")
        .map(|seg| format!("/{seg}"))
        .collect::<Vec<_>>();
    roots.sort();
    roots.dedup();
...
async fn discover_photonvision_streams(
    peers: &PeersServiceState,
    host: &str,
    base_port: u16,
    max_streams: u16,
...
async fn probe_helios_api(peers: &PeersServiceState, base: &str, timeout_ms: u64) -> ProbeResult {
    ...
    let candidate = if base.ends_with("/v1") {
        format!("{base}/device/hostname")
    } else {
        format!("{base}/v1/device/hostname")
    };
    let start = std::time::Instant::now();
    let request = peers
        .probe_client()
            ...
            let body_ok = resp
                ...
                .and_then(|value| {
                    value
                        ...
                        .map(|hostname| !hostname.trim().is_empty())
                })
                .unwrap_or(false);
            ProbeResult {
                url: candidate,
