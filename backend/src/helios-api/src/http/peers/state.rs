...
use super::probe::{
    apply_integration_defaults, default_integration_metadata, derive_endpoints,
    infer_discovered_integration_kind, normalize_api_base, normalize_device_host,
};
...
impl Default for PeersServiceState {
    fn default() -> Self {
        Self {
            ...
            http_client: crate::http::reqwest_client::build_http_client("HeliOS/peers")
                .expect("reqwest client"),
            probe_client: crate::http::reqwest_client::build_http_client("HeliOS/peers-probe")
                .expect("reqwest client"),
        }
    }
}
...
fn peers_state_paths() -> (PathBuf, Option<PathBuf>) {
    match std::env::var_os("HELIOS_PEERS_FILE") {
        ...
        None => (
            persisted_files::data_root_file("peers.json"),
            Some(persisted_files::legacy_helios_etc_file("peers.json")),
        ),
    }
}
...
impl PeersServiceState {
    ...
    pub(crate) async fn snapshot_peer_streams(
        &self,
        state: &AppState,
        ...
        let now = Instant::now();
        {
            ...
            if let Some(entry) = cache.as_ref()
                && now.saturating_duration_since(entry.fetched_at)
                    < std::time::Duration::from_secs(PEER_STREAM_CACHE_TTL_SECS)
            {
                return PeerRemoteStreamsResponse {
                    streams: entry.streams.clone(),
        ...
        let entry = PeerStreamCacheEntry {
            fetched_at: now,
            fetched_at_rfc3339: fetched_at.clone(),
...
pub(crate) async fn list_peers(State(state): State<AppState>) -> impl IntoResponse {
    ...
    Json(PeerInventoryResponse {
        peers: peer_state.peers.clone(),
        discovery: peer_state.discovery.clone(),
    })
}
...
pub(crate) async fn register_peer(
    ...
    let integration_kind = req
        .integration
        .as_ref()
        .map(|integration| integration.kind.clone())
        .unwrap_or(PeerIntegrationKind::Helios);
    let api_base_url = match normalize_api_base(
        req.api_base_url.as_deref(),
        req.device_ip.as_deref(),
        &integration_kind,
    ) {
        Ok(value) => value,
        Err(err) => {
    ...
    let mut integration = req
        .integration
        .unwrap_or_else(|| default_integration_metadata(integration_kind.clone()));
    ...
    if let Err(err) = persist_peers_to_disk(peers_for_disk).await {
        return (
            ...
            Json(PeerError {
                error: format!("peer registered in memory but failed to persist: {err}"),
            }),
        )
            .into_response();
    }
...
pub(crate) async fn discover_peers(
    State(state): State<AppState>,
    Json(request): Json<PeerDiscoveryRequest>,
) -> impl IntoResponse {
    let scopes = if request.scopes.is_empty() {
        vec![PeerDiscoveryScope::Mdns, PeerDiscoveryScope::Broadcast]
    } else {
        request.scopes.clone()
    };
    let timeout_secs = request.timeout_secs.unwrap_or(5);
    let started_at = Utc::now();
    let expected_completion = started_at
        .checked_add_signed(Duration::seconds(timeout_secs as i64))
        .map(|time| time.to_rfc3339());
    let run_id = uuid::Uuid::new_v4().to_string();
    let discovery = PeerDiscoveryResponse {
        run_id,
    ...
    if scopes
        .iter()
        .any(|scope| matches!(scope, PeerDiscoveryScope::Mdns))
    {
    ...
    if scopes
        .iter()
        .any(|scope| matches!(scope, PeerDiscoveryScope::Broadcast))
    {
    ...
    let local_addresses: HashSet<_> = lib_net::interface::get_local_addresses()
        .await
        .unwrap_or_default()
        .into_iter()
        .collect();
    ...
    if !discovered_hosts.is_empty() {
        let peers_for_disk = {
            ...
            peer_state
                .peers
                .retain(|peer| !peer_matches_any_ip(peer, &local_addresses));

            let now = Utc::now().to_rfc3339();
            let classification_timeout_ms =
                ((timeout_secs.saturating_mul(1000)) / 2).clamp(300, 1500);
            for (host_ip, seen_ports) in discovered_hosts {
                let host = host_ip.to_string();
                let Some(integration_kind) = infer_discovered_integration_kind(
                    state.services.peers.inner(),
                    &host,
                    &seen_ports,
                ...
                if let Some(existing) = peer_state.peers.iter_mut().find(|peer| {
                    peer.api_base_url == api_base_url
                        || peer
                            .endpoints
                            .iter()
                            .any(|endpoint| endpoint.host == host.as_str())
                }) {
                    ...
                    if existing.endpoints.is_empty()
                        || !existing
                            ...
                            .any(|endpoint| endpoint.host == host.as_str())
                    {
                        existing.endpoints.push(PeerEndpoint {
                            host: host.clone(),
                            port: Some(api_port),
                        });
                    }
                    if !matches!(
                        existing.integration.kind,
                        PeerIntegrationKind::Custom | PeerIntegrationKind::Photonvision
                    ) && matches!(integration_kind, PeerIntegrationKind::LimelightOs)
                    {
                        existing.integration.kind = PeerIntegrationKind::LimelightOs;
                    }
                    if existing.integration.management_url.is_none()
                        && integration.management_url.is_some()
                    {
                        existing.integration.management_url = integration.management_url.clone();
                    }
                    if existing.integration.stream_url.is_none()
                        && integration.stream_url.is_some()
                    {
                        existing.integration.stream_url = integration.stream_url.clone();
                    }
                    if existing.integration.stream_urls.is_empty()
                        && !integration.stream_urls.is_empty()
                    {
        ...
        if let Err(err) = persist_peers_to_disk(peers_for_disk).await {
            return (
                ...
                Json(PeerError {
                    error: format!(
                        "peer discovery updated memory state but failed to persist: {err}"
                    ),
                }),
            )
                .into_response();
...
pub(crate) async fn remove_peer(
    ...
    if let Err(err) = persist_peers_to_disk(peers_for_disk).await {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(PeerError {
                error: format!("peer removed from memory but failed to persist: {err}"),
            }),
        )
            .into_response();
    }
...
fn peer_matches_any_ip(
    ...
    if peer
        .endpoints
        .iter()
        .filter_map(|endpoint| endpoint.host.parse::<std::net::IpAddr>().ok())
        .any(|ip| addresses.contains(&ip))
    {
        return true;
    }

    Url::parse(&peer.api_base_url)
        .ok()
        .and_then(|url| {
            url.host_str()
                .and_then(|host| host.parse::<std::net::IpAddr>().ok())
        })
        .is_some_and(|ip| addresses.contains(&ip))
}
...
fn upsert_peer(peers: &mut Vec<PeerInfo>, peer: PeerInfo) {
    if let Some(index) = peers
        .iter()
        .position(|existing| existing.id == peer.id || existing.api_base_url == peer.api_base_url)
    {
