...
fn subscribe_requires_valid_kinds() {
    ...
    let error = handle_client_message(r#"{"op":"subscribe","kinds":["bogus"],"interval_ms":250}"#, &mut requested, &mut min_interval).unwrap_err();
...
fn subscribe_populates_requested_kinds_and_interval() {
    ...
    handle_client_message(r#"{"op":"subscribe","kinds":["imu","power","firmware"],"interval_ms":500}"#, &mut requested, &mut min_interval).unwrap();
