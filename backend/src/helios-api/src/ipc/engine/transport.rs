...
fn build_engine_connection(stream_count: usize, connected_initial: bool) -> (EngineConnection, mpsc::Receiver<EngineRequest>) {
    ...
    let timeout_scale_ppm = Arc::new(AtomicU64::new(HELIOS_ENGINE_IPC_POLICY.resolve().timeout_scale_for_streams(stream_count)));
...
fn resolve_engine_sockets() -> Vec<PathBuf> {
    HELIOS_ENGINE_IPC_POLICY.resolve().client_socket_candidates(Path::new(env!("CARGO_MANIFEST_DIR")))
}
