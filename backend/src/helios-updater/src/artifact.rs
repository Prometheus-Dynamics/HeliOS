...
pub struct ReleaseManifest {
    ...
    ///
    /// This is kept as a plain string because updater metadata is intentionally
    /// opaque JSON at the service boundary, and `serde_json::Value`
    /// `deserialize_any` support does not map cleanly onto the archive-native
    /// IPC payload rules.
    #[serde(default)]
