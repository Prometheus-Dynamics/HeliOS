...
use crate::localization::math::{compose_transforms, invert_transform, transform_to_pose, transform_to_rotation, transform_to_translation};
                        ...
                        match robot_field_pose_from_camera_sample(sample, rig_poses) {
                            Ok(Some(robot_pose)) => {
                                if robot_pose.has_translation {
                                    robot_estimates.push((robot_pose.pose, sample.source.weight, sample.source.id.clone()));
