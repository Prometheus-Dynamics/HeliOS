...
use lib_storage_layout::{LayoutPartition, SizeSource, StorageLayoutManifest};
...
fn compute_mirrored_slot_size_mib(layout: &StorageLayoutManifest, disk: &str, sector_bytes: u64) -> Result<u64> {
    let source = layout.slot_a().map_err(|err| anyhow!(err))?;
...
impl From<lib_storage_layout::PartitionMode> for crate::config::Mode {
    fn from(value: lib_storage_layout::PartitionMode) -> Self {
        match value {
            lib_storage_layout::PartitionMode::Resize => Self::Resize,
            lib_storage_layout::PartitionMode::Mkpart => Self::Mkpart,
            lib_storage_layout::PartitionMode::Noop => Self::Noop,
        }
    }
}
...
#[cfg(test)]
mod tests {
    use super::resolve_mirror_existing_size_mib;
    ...
    #[test]
    fn resolves_ext4_layout_fixture() {
        let layout = StorageLayoutManifest::load_from_path(&fixture("ext4-labels.toml")).expect("ext4 layout");
        assert_eq!(layout.layout_id, "ext4_labels");
        assert_eq!(layout.slot_a().unwrap().name, "ACTIVE");
        assert_eq!(layout.slot_b().unwrap().name, "RESERVE");
    }
