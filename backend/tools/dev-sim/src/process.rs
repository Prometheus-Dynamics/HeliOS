...
pub async fn spawn_updater(exe: &Path, config: &PathConfig, args: &Args) -> AnyResult<ManagedProcess> {
    ...
    cmd.stdin(Stdio::null())
        ...
        .env("HELIOS_UPDATER_JOURNAL_PATH", updater_paths.journal_str())
        .env("HELIOS_UPDATER_DATA_DIR", updater_paths.data_dir_str())
        .env("HELIOS_UPDATER_CACHE_DIR", updater_paths.cache_dir_str())
        .env("HELIOS_UPDATER_WORK_DIR", updater_paths.work_dir_str())
        .env("LOG_FOLDER", config.api.log_dir_str())
        .env("HELIOS_MEDIA_LIBRARY_PATH", config.media.root_str())
