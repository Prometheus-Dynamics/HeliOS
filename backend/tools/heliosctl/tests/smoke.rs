...
async fn updater_command_smoke() -> Result<()> {
    ...
    let stage_output = Command::cargo_bin("heliosctl")?.args(["--rpc-url", &rpc_url, "updater", "stage", "https://example.com/manifest.json"]).output()?;
    ...
    let apply_output =
        Command::cargo_bin("heliosctl")?.args(["--rpc-url", &rpc_url, "updater", "apply", &UPDATE_ID.to_string(), "--window-start", TEST_TIMESTAMP, "--window-duration-secs", "3600"]).output()?;
    ...
    let cancel_output = Command::cargo_bin("heliosctl")?.args(["--rpc-url", &rpc_url, "updater", "cancel", &UPDATE_ID.to_string()]).output()?;
    ...
    let rollback_output = Command::cargo_bin("heliosctl")?.args(["--rpc-url", &rpc_url, "updater", "rollback", &UPDATE_ID.to_string()]).output()?;
    ...
    let sync_output = Command::cargo_bin("heliosctl")?.args(["--rpc-url", &rpc_url, "engine", "sync"]).output()?;
    ...
    let reset_output = Command::cargo_bin("heliosctl")?.args(["--rpc-url", &rpc_url, "engine", "reset", "--reason", "crash-recovery"]).output()?;
    ...
    let timeline_output = Command::cargo_bin("heliosctl")?.args(["--rpc-url", &rpc_url, "updater", "timeline", "--limit", "5"]).output()?;
